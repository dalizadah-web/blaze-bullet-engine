...
TEST_CASE(transposition_table_round_trips_complete_entry) {
    ...
    table.store(0x123456789abcdef0ULL, move, 37, 8, Bound::Exact, 2);
...
TEST_CASE(transposition_table_distinguishes_bounds_and_misses) {
    TranspositionTable table(1);
    table.store(1, Move(Square::A2, Square::A3), -12, 4, Bound::Upper, 0);
...
TEST_CASE(transposition_table_normalizes_mate_distance) {
    TranspositionTable table(1);
    table.store(9, Move(Square::H2, Square::H3), 31990, 12, Bound::Lower, 5);
...
TEST_CASE(transposition_table_clear_and_resize_remove_entries) {
    TranspositionTable table(1);
    table.store(7, Move(Square::B1, Square::C3), 5, 3, Bound::Exact, 0);
...
TEST_CASE(transposition_table_generation_preserves_new_deep_entries) {
    ...
    table.store(42, deep, 20, 14, Bound::Exact, 0);
    table.new_search();
    table.store(42, Move(Square::G1, Square::H3), 10, 2, Bound::Upper, 0);
    ...
    for (std::uint64_t worker = 0; worker < 4; ++worker) {
        workers.emplace_back([&, worker] {
            for (std::uint64_t iteration = 0; iteration < 20000; ++iteration) {
                ...
                table.store(key, move, expected, static_cast<int>(key % 32), Bound::Exact, 0);
