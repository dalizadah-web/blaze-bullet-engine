...
The empty dependency and asset arrays are deliberate for the current source-only release build. The Blaze network loader and Polyglot reader are format adapters only; no network or book is shipped, enabled, or trusted by default. Any future library, network, book, or tablebase asset must record its origin, license, relative path, and SHA-256 before it may enter a release build. Rating-run manifests additionally bind the source revision, compiler and flags, executable, datasets, and hardware used for the run.
